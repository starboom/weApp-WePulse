...
logging.basicConfig(level=logging.INFO,
	...
	filename='/home/pi/Desktop/heartdemo/heartpi.log',
	filemode='a')
...
#        print N, Signal, curTime, sampleCounter, lastBeatTime
		...
		continue
	      ...
	      if count >= 0 :
      	      	post_data = {"pulse" : int(lastBPM)}
              	postfields = urlencode(post_data)
              	c.setopt(c.POSTFIELDS, postfields)
	      	c.perform()
              	count = 0
              	logging.info("catch pulse %d ,send to SERVER", lastBPM)
              	time.sleep(1) #sleep for server for response
